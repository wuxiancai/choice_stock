...
- 当日推荐采用日线“四重共振”策略，四项为入池硬条件，并保留两个不重叠维度纳入 100 分：均线趋势 30、MACD 动能 20、RSI 位置 15、成交量确认 15、九转时点 10、资金强度 10。金叉日必须为 MA5 上穿 MA20、双均线上倾且收盘站上 MA20；MACD 要求 DIF>DEA、红柱放大且 DIF 位于或上穿零轴；RSI 必须曾低于 30、确认日回升至 50–70；金叉日量至少为前 5 日均量 1.2 倍，随后 3 个交易日不低于各自前 5 日均量。确认日九转仅接受上行 3–5，主力资金必须净流入；资金分仍按 `(大单+特大单净流入) / 当日成交额`，不以绝对金额偏向大市值股票。OBV、量比、换手率仅作资金确认或风险提示，禁止重复加分。由于后续 3 日确认，推荐显示确认日并单列金叉日。当前只支持日线，不将日线结果伪装为 60 分钟周期。
